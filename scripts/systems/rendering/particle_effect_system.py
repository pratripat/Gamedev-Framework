...
class ParticleEffectSystem:
    ...
    def update(self, dt, quadtree=None, camera_rect=None):
        ...
        cull_rect = camera_rect.inflate(300, 300) if camera_rect else None
        dt_60 = dt * 60.0
        friction_cache = self._friction_cache
        ...
        for read_pos in range(len(self.active_indices)):
            ...
            if p.friction < 1.0:
                factor = friction_cache.get(p.friction)
                if factor is None:
                    factor = p.friction ** dt_60
                    friction_cache[p.friction] = factor
                p.vx *= factor
                p.vy *= factor
